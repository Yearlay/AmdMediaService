...
public class RoundImageViewByXfermode extends ImageView {
	...
	public RoundImageViewByXfermode(Context context, AttributeSet attrs) {
		...
		mBorderRadius = a.getDimensionPixelSize(R.styleable.RoundImageViewByXfermode_borderRadius, (int) TypedValue
				.applyDimension(TypedValue.COMPLEX_UNIT_DIP, BODER_RADIUS_DEFAULT, getResources().getDisplayMetrics()));
		type = a.getInt(R.styleable.RoundImageViewByXfermode_type, TYPE_CIRCLE); // moren为 ROUND
		a.recycle();
...
//					drawable.setBounds(0, 0, (int) scale * dWidth, (int) scale * dHeight);
					...
					if (mWeakBitmap != null) {
						Bitmap wBitmap = mWeakBitmap.get();
						if (wBitmap != null) {
							if (!wBitmap.isRecycled()) {
								 wBitmap.recycle();
								 wBitmap = null;
							}
						}
...
//						bitmap.getWidth()/2-25, bitmap.getHeight()-8, paint);
				return;
			}
	}
	...
	String TAG = "RoundImageViewByXfermode";
