...
public class VROperatorReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context arg0, Intent intent) {
		...
		if (VRApp.ACTION_APP.equals(action)) {
			...
			startServiceForVRRadio(arg0, intent.getIntExtra(VRRadio.KEY_COMMAND_CODE, 0));
	...
	public void startServiceForVRRadio(Context context, int commandCode) {
		DebugLog.d(TAG, " startServiceForVRRadio commandCode: " + commandCode);
		if (commandCode != 0) {
			Intent intents = new Intent(context, MediaService.class);
			intents.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
